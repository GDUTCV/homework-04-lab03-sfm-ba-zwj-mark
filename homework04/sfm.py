...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    """ YOUR CODE HERE """
    reprojected_points, _ = cv2.projectPoints(points3d, rotation_mtx, tvec, intrinsics, distCoeffs=None)
    reprojected_points = reprojected_points.reshape(-1, 2)

    residuals = np.linalg.norm(points2d - reprojected_points, axis=1)
    """ END YOUR CODE HERE """
    return residuals
...
def add_points3d(image_id1: str, image_id2: str, all_extrinsic: dict, intrinsics, points3d: np.ndarray,
    ...
    """
    proj_mtx1 = intrinsics @ all_extrinsic[image_id1]
    proj_mtx2 = intrinsics @ all_extrinsic[image_id2]

    points2d_1 = get_selected_points2d(image_id=image_id1, select_idxs=matches[:, 0])
    points2d_2 = get_selected_points2d(image_id=image_id2, select_idxs=matches[:, 1])

    homogeneous_points3d = cv2.triangulatePoints(proj_mtx1, proj_mtx2, points2d_1.T, points2d_2.T)

    new_points3d = homogeneous_points3d[:3, :] / homogeneous_points3d[3, :]
