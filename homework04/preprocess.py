...
def create_ransac_matches(image_file1: os.path, image_file2: os.path,
    ...
    """
    essential_mtx, is_inlier = cv2.findEssentialMat(points1, points2, camera_intrinsics, method=cv2.RANSAC,
                                                       prob=0.999, threshold=ransac_threshold)
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    for i, image_file1 in enumerate(image_files):
        for j, image_file2 in enumerate(image_files):
            if i >= j:
                continue
            match_id = f"{os.path.basename(image_file1)[:-4]}_{os.path.basename(image_file2)[:-4]}"
