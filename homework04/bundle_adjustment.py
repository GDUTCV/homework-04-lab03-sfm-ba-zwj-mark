...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    """
    selected_rvecs = camera_rvecs[camera_idxs]
    selected_tvecs = camera_tvecs[camera_idxs]
    selected_points3d = points3d[points3d_idxs]

    points_cam = np.einsum('ijk,ik->ij', extrinsics[camera_idxs, :, :3], selected_points3d) + selected_tvecs

    points_proj = np.matmul(intrinsics, points_cam.T).T  # N x 3

    points_proj = (points_proj[:, :2].T / points_proj[:, 2]).T  # N x 2

    residuals = np.sqrt(np.sum(np.square(points_proj - points2d), axis=1))
